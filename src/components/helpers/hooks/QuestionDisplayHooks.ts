...
export default function useIsOnlyField(callerDisplayOrder = null, refreshTrigger = null) {
  ...
  const [onlyFieldDetails, setOnlyFieldDetails] = useState({
    isOnlyField: false,
    overrideLabel: ''
  });
  ...
  useEffect(() => {
    ...
    if (DFContext.displayAsSingleQuestion) {
      defaultOnlyFieldDetails.isOnlyField =
        callerDisplayOrder === '0' ? DFContext.displayAsSingleQuestion : false;
    }
    // Otherwise, use the Assignment context's singleQuestion page value, to fall back to the original logic (checking number of editable fields);
      ...
      const editableFieldsCount = PCore.getFormUtils().getEditableFields(
        PCore.getContainerUtils().getActiveContainerItemContext(`${context}/workarea`)
      ).length;
      ...
      if (editableFieldsCount === 1 && !pageHasNonEditableField) {
        ...
        defaultOnlyFieldDetails.isOnlyField = !!PCore.getStoreValue(
          'displayAsSingleQuestion',
          '',
          'app'
        );
      }
...
function registerComplexQuestion() {
  useEffect(() => {
    if (!PCore.getStoreValue('isComplexQuestionPage', '', 'app')) {
      PCore.getStore().dispatch({
        type: 'SET_PROPERTY',
        payload: {
          reference: 'isComplexQuestionPage',
          value: true,
          context: 'app',
          isArrayDeepMerge: true
        }
...
export { registerComplexQuestion };
